...
export default function TransactionModal({ isOpen, onClose, isAnalyzing, isSaving, formData, onInputChange, onImageUpload, onSave, triggerFileInput }) {
  const fileInputRef = useRef(null);

  return isOpen ? (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#0f172a] border border-slate-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-[#1e293b]/30">
          <h3 className="text-xl font-bold text-white flex items-center gap-2">
            <Plus className="text-blue-500"/> Record Transaction
          </h3>
        ...
        <div className="p-6 flex flex-col gap-6">
          ...
          <div className="space-y-4">
            ...
            <button 
              onClick={onSave}
              disabled={isSaving}
              className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3.5 rounded-xl transition-all shadow-lg hover:shadow-blue-500/30 flex justify-center items-center gap-2 mt-4"
            >
              {isSaving ? <Loader2 className="animate-spin" size={20} /> : null}
              {isSaving ? "Saving to Database..." : "Save Transaction"}
            </button>
          </div>
        </div>
      </div>
