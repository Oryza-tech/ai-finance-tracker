export default function ExpenseCategories({ sortedCategories, totalExpense, formatRupiah }) {
  return (
    <div className="col-span-1 bg-[#0f172a] p-6 rounded-2xl border border-slate-800 shadow-lg flex flex-col justify-between h-80">
      <div>
        <h4 className="text-white font-bold text-lg">Expense Categories</h4>
        <p className="text-slate-400 text-xs mt-0.5">Your biggest spending categories.</p>
      </div>
      <div className="flex flex-col gap-4 mt-4 justify-end flex-1">
        {sortedCategories.length > 0 ? (
          sortedCategories.map(([cat, val], idx) => {
            const percentage = Math.min((val / totalExpense) * 100, 100) || 0;
            return (
              <div key={idx} className="w-full">
                <div className="flex justify-between text-xs font-medium text-slate-300 mb-1.5 capitalize">
                  <span>{cat}</span>
                  <span className="text-slate-400">{formatRupiah(val)}</span>
                </div>
